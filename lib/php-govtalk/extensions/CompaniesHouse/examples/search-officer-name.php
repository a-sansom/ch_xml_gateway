...
if (isset($_GET['officersurname']) && isset($_GET['officerforename'])) {
	...
	if ($officerList = $companiesHouse->companyOfficerSearch($_GET['officersurname'], $_GET['officerforename'], 'CUR', null, $_GET['resigned_ind'])) {
	 ...
	 // Similar (including nearest match)...
		...
		foreach ($officerList['match'] AS $officer) {
			echo '<li>'.$officer['title'].' '.$officer['forename'].' '.$officer['surname'].' (<a href="view-officer-details.php?id='.urlencode($officer['id']).'">View</a>)</li>';
		}
		echo '</ul>';
...
<form action="" method="get">
	Search for officer (forename / surname): <input name="officerforename" type="text" /> <input name="officersurname" type="text" /> 
	Resigned indicator: 0 <input type="radio" name="resigned_ind" value="0" checked="checked" /> 1 <input type="radio" name="resigned_ind" value="1" />
